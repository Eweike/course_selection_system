...
public class Tools {
	...
	public static String getPeopleNameRandom() {
	...
	public static String getLessonNameRandom() {
		Random random = new Random(System.currentTimeMillis());
		/* 598 百家姓 */
		String[] Surname1 = {"Java","PHP","C++","C#","C语言","JavaScript",".NET","Python","MySql","SqlServer","Oracle"};
		String[] Surname2 = {"基础与案例开发","入门与实践","整编教材","从入门到放弃","从删库到跑路","高级程序设计","与颈椎病康复指南","设计母猪的产后护理"};
		String[] Surname3 = {"第一版","第二版","第三版"};
		
		String name1 = Surname1[random.nextInt(Surname1.length)];
		String name2 = Surname2[random.nextInt(Surname2.length)];
		String name3 = Surname3[random.nextInt(Surname3.length)];
	...
	public static String getPeopleSexRandom() {
		String sex = null;
		Random random = new Random();
		if (random.nextBoolean()) {
			sex = "男";
	...
	public static Map getAddress() {
		Map map = new HashMap();
		map.put("name", getPeopleNameRandom());
		map.put("sex", getPeopleSexRandom());
	 ...
	 */
	/*
	 * private static String name_sex = ""; private static String
	 * getChineseName() { int index=getNum(0, firstName.length()-1); String
	 * first=firstName.substring(index, index+1); int sex=getNum(0,1); String
	 * str=boy; int length=boy.length(); if(sex==0){ str=girl;
	 * length=girl.length(); name_sex = "女"; }else { name_sex="男"; }
	 * index=getNum(0,length-1); String second=str.substring(index, index+1);
	 * int hasThird=getNum(0,1); String third=""; if(hasThird==1){
	 * index=getNum(0,length-1); third=str.substring(index, index+1); } return
	 * first+second+third; }
	 */
}
